...
function printPath ($target, $linkID, &$jsonMessage, $userID) {
  ...
  foreach ($pathReverse as $node) {
    ...
    if ($i > 0) {//exclude the starting point
      $sqlNodeInfo = mysqli_query($linkID, 
        "SELECT nodeID, I_E, Latitude, Longitude
         FROM Nodes
      ...
      $sqlEdgeAttributes = mysqli_query($linkID, 
        "SELECT *
         FROM EdgeAttributes
         WHERE edgeID = '$edgeID'");
  ...
  if ($numA_Edges <= 0) {
    ...
    for ($i=0; $i < $numA_Edges; $i++) {
      ...
      if ($PathType != "Exit only" || $Obstacles == "A to B") {
        ...
        if (mysqli_num_rows($nodeBInfo) <= 0) {
  ...
  if ($numB_Edges <= 0) {
    ...
    for ($i=0; $i < $numB_Edges; $i++) {
      ...
      if ($PathType != "Exit only" || $Obstacles == "B to A") {
        ...
        if (mysqli_num_rows($nodeAInfo) <= 0) {
...
function createStart ($linkID, &$jsonMessage, $userID) {
  ...
  $currentPosID = mysqli_insert_id($linkID);
  ...
  if ($closestPoint->nodeID == $minEdge->endPointA->nodeID) {
    ...
    $tempEdgeID = mysqli_insert_id($linkID);
  ...
  //if closest point lies on the edge, add it to DB along with three temp edges
    ...
    $closestPointID = mysqli_insert_id($linkID);
    ...
    $perpEdgeID = mysqli_insert_id($linkID);
...
function createTarget ($linkID, &$jsonMessage) {
  ...
  if (mysqli_num_rows($targetNodeInfo) <= 0) {
